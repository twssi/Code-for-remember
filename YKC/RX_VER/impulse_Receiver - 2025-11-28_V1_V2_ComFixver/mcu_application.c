...
static cx_uint8_t transmit_relay_input_value 		= 0u;
...
void calculate_inputcapture_TIM3 (void)
{
	...
	else if(_freq_state == DONE)
	{
		...
		{ 
			...
			/* Compute frequency with integer rounding to reduce truncation error */
			gu32_Freq = (uint32_t)((F_CLK + (gu32_T1H/2)) / gu32_T1H);
			de_rx_frequency = gu32_Freq;
			_freq_state = IDLE;
			_freq_count = 0;

			/* Apply measurement tolerance around 300 (3.00Hz) to correct small systematic error */
			if (gu32_Freq ==299 || gu32_Freq == 301)
			{
				_freq_data = 300;
			}
			else
...
static void control_watch (void)
{
	...
	if( (get_relay_voltage_V1 >= 200) && (get_relay_voltage_V2 >= 300) )	
	{
...
static void control_input(void)
{
	...
	else
	{
		if(CX_FALSE == relay_input)
		{
			...
			if(get_relay_voltage_V1 > 200 && get_relay_voltage_V2 > 350)	
			{
