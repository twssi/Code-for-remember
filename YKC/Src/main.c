...
void COM_Input_Parse1(cx_uint8_t* bufptr, cx_uint_t len, cx_uint_t com_port)
{
	...
	if (com_port == 1)  // COM1 처리
    {
		...
		memcpy(&parsebuf[0], &prevparsebuf[0], 46);
		...
		if(parsebuf[22] == 0x30)
		{
			...
			{
				bootup_flag++;
				main_pulse_value = 380 - (bootup_flag * 10) + m_val%30 - 15; // 기본 380 ~ 350 +- 15
				//sub_pulse_value = 80 - (bootup_flag * 20) + m_val%30 - 15; // 기본 80 ~ 20 +- 15
				//변환이 필요한 특정 바이트 값 오버라이트
				for(i=0; i<3; i++)
				{
					prev_parsebuf[22+i]= main_pulse_value/POW(10,2)%10+'0';
					parsebuf[22+i] = prev_parsebuf[22+i];
				}
							
			}
		}

		if (parsebuf[25] == 0x30) // 수신 데이터 초기화 시
		{
			//memcpy(&parsebuf[0], &prevparsebuf[0], 46);
			if (bootup_flag < 4)
			{
				...
				for(i=0; i<3; i++)
				{
					prev_parsebuf[22+i]= sub_pulse_value/POW(10,2)%10+'0';
					parsebuf[22+i] = prev_parsebuf[22+i];
				}
			}
		}
        ...
        memcpy(&_DG_tx_buf[1][0],&parsebuf[len-46], 26);
...
void COM_Input_Parse2(cx_uint8_t* bufptr, cx_uint_t len, cx_uint_t com_port)
{
	...
	if (com_port == 2)
	{
		...
		memcpy(&_DG_tx_buf[2][0],&parsebuf[len-46], 26);
...
void COM_Input_Parse3(cx_uint8_t* bufptr, cx_uint_t len, cx_uint_t com_port)
{
	...
	if (com_port == 3)
	{
		...
		memcpy(&_DG_tx_buf[3][0],&parsebuf[len-46], 26);
...
void COM_Input_Parse4(cx_uint8_t* bufptr, cx_uint_t len, cx_uint_t com_port)
{
	...
	if (com_port == 4)
	{
		...
		memcpy(&_DG_tx_buf[4][0],&parsebuf[len-46], 26);
...
void COM_Input_Parse5(cx_uint8_t* bufptr, cx_uint_t len, cx_uint_t com_port)
{
	...
	if (com_port == 5)
	{
		...
		memcpy(&_DG_tx_buf[5][0],&parsebuf[len-46], 26);
...
void COM_Input_Parse6(cx_uint8_t* bufptr, cx_uint_t len, cx_uint_t com_port)
{
	...
	if (com_port == 6)
	{
		...
		memcpy(&_DG_tx_buf[6][0],&parsebuf[len-46], 26);
...
void COM_Input_Parse7(cx_uint8_t* bufptr, cx_uint_t len, cx_uint_t com_port)
{
	...
	if (com_port == 7)
	{
        ...
        memcpy(&_DG_tx_buf[7][0],&parsebuf[len-46], 26);
...
void COM_Input_Parse8(cx_uint8_t* bufptr, cx_uint_t len, cx_uint_t com_port)
{
	...
	if (com_port == 8)
	{
		...
		memcpy(&_DG_tx_buf[8][0],&parsebuf[len-46], 26);
