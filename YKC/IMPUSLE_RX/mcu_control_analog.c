...
void get_input_data_relay_voltage(cx_bool_t active_relay_measure, cx_uint_t relay_count_value)
{
	...
	{
		_array_PA_2[relay_count_value-1] = _ADC_ValueTab[2];	//V1 전압
        _array_PA_3[relay_count_value-1] = _ADC_ValueTab[3];	//V2 전압
	}	
}
...
void analog_100us_tim5_irq(void) // 실제 50us 마다 인터럽트
{
	put_current_value();
}
...
//-----------------------------------------------------------------------	
//rx current
void put_current_value(void) // dma 측정할때마다 호출
{
    //-----------------------------------------------------------------------
	...
	if(_count_rx_current >= 20u) 
    {
		...
		if(_count_average_current >= SAMPLING_COUNT_AVERAGE_CURRENT) 
		{
			...
			_voltage_Level_rx_current 	= (cx_float_t)((temp_current_value*3.3)/(4096-1));	//전류 전압레벨
...
void control_watch_pulse_frequency(pulse_frequency_t* data, cx_float_t pulse_frequency)
{
    ...
    if (CX_TRUE==fail)
	{
        if (data->sampling_pulse_frequency.fail_count<=CHATTERING_COUNT_PULSE_FREQ) //5번
		{
